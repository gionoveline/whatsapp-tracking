-- Allow global admin to create companies even when email domain is already auto-linked.
-- Behavior: when conflict happens, global admin creation continues without auto domain link.
...
declare
  ...
  v_should_auto_link boolean := coalesce(p_auto_link_by_domain, false);
begin
  ...
  if v_should_auto_link then
    ...
    if exists (
      ...
      where p.auto_link_by_domain = true
        and p.allowed_email_domain = v_domain
    ) then
      if coalesce(p_user_is_global_admin, false) then
        -- Super admin can still create the company; only disable auto domain link for this partner.
        v_should_auto_link := false;
        v_domain := null;
      else
  ...
  for i in 0..99 loop
    ...
    begin
      ...
      values (
        ...
        v_should_auto_link,
        case when v_should_auto_link then v_domain else null end
      )
